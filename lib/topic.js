...
exports.page = function (request, response) {
    ...
    db.query('SELECT * FROM topic', function (error, topics) {
        ...
        db.query(`SELECT * 
            ...
            LEFT JOIN author a 
            ON t.author_id = a.id 
            WHERE t.id=?`, [queryData.id], function (error2, topic) {
            // sql injection을 막기위해 ?로 하고 2번째 인자에 배열로 적어줌. 치환하면서 문자열처리
            if (error2) {
                throw error2;
            }
            console.log(topic);
        ...
        VALUES(?, ?, NOW(), ?)`, [post.title, post.description, post.author],
            function (error, result) {
                ...
                response.writeHead(302, {
                    Location: `/?id=${result.insertId}` // result.insertId <- 새로 등록될 행의 id
                });
                response.end();
            });
    });
